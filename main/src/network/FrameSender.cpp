...
spsc_queue<cv::Mat> FrameSender::sendQueue(512);

void FrameSender::addToQueue(cv::Mat frame) {
...
void FrameSender::run() {
    ...
    cv::Mat latestFrame;
    while (ThreadFlags::FRAME_SENDER) { //While the threadmanager says this thread should run
